module.exports = async function updateKnownFriends({
  ...
  for (const player of json.players) {
    ...
    await db.friends.update({_id: knownFriend._id}, {$set: {
  ...
  if (knownFriendCacheBefore) {
    knownFriendCache = await db.known_friends.update({_id: knownFriendCacheBefore._id}, {$set: knownFriendCache});
  } else {
