...
function overviewStatus(accounts: UiAccounts[], step: WingsResStep, wing: WingsRef) {
  ...
  if (
    ...
    wing.isStrike &&
    accounts.find((a) => a.completedStrikesDaily?.[firstTrigger(step)])
  ) {
    completed = true;
  } else if (
    wing.isFractal &&
...
<template>
  <main id="overview" :class="{ 'with-bg': !data.baseConfig?.boringBg }">
      ...
      <div class="wing" v-for="wing in wings">
        ...
        <a
          ...
          <svg class="wing-boss-border" viewBox="0 0 300 300">
            <path
              v-for="p in svgBossBorder(data.accounts, step, wing)"
              :d="p.d"
          ...
          <div class="wing-step-tooltip">
                ...
                <span
                  ...
                  :class="{
                    ...
                    'wing-step-tooltip-mark-open': !(
                      acc.completedStrikesDaily &&
                      acc.completedStrikesDaily[firstTrigger(step)]
                    ),
                  }"
                >
                  <img
            ...
            >
              ...
              <div v-for="acc in data.accounts">
                <span
                  ...
                  :class="{
                    'wing-step-tooltip-mark': true,
                    'wing-step-tooltip-mark-open': !acc.completedStrikesDaily?.[
                      firstTrigger(step)
                    ],
                  }"
                >
            ...
            >
            <span v-else class="li-display-number-details">{{
              i18n.weeklyResetInfo
            }}</span>
            <span
              v-if="isDailyToday(wing, step, data.dayOfYear)"
              class="li-display-number-details"
    ...
    <div class="li-section">
      ...
      <div class="li-display" v-if="data.zhaitaffy">
        <img
          class="li-display-img"
          :src="img('./img/591443.png')"
          :alt="i18n.labelZhaitaffy1000"
        />
        <span class="li-display-number">{{
          Math.floor(data.totalKps.zhaitaffy / 1000)
        }}</span>
        <img
          class="li-display-img"
          :src="img('./img/591442.png')"
    ...
    <template
      v-for="acc in data.accounts.filter(
        (acc) =>
          acc.kps?.unopenedBoxes &&
          acc.kps.unopenedBoxes.length > 0 &&
          acc.accountInfo?.name
      )"
    >
        ...
        <template v-for="step in wing.steps">
          ...
          >
            <div class="li-display">
              ...
              <span class="li-display-number"
                >{{ box.item.count }}x
                <img class="li-display-img" :src="img('./img/1302744.png')"
              /></span>
            </div>
          </template>
        </template>
