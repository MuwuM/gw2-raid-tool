...
function onResize() {
  const logDisplays = document.querySelectorAll('.arc-log-display') as Iterable<HTMLElement>
...
<template>
  <nav class="navbar navbar-expand navbar-dark bg-primary sticky-top">
    <ul class="navbar-nav mr-auto">
      <template v-if="data.accounts.length >= 1">
        ...
        <li class="nav-item active">
          <a class="nav-link" href="#" tabindex="-1" @click="selectPage('logs', {}, $event)">{{
            i18n.navArcdps
          }}</a>
        </li>
        <li class="nav-item active">
          <a class="nav-link" href="#" tabindex="-1" @click="selectPage('friends', {}, $event)">{{
            i18n.navFriends
          }}</a>
        </li>
        <li class="nav-item active">
      ...
      <li class="nav-item active">
        <a class="nav-link" href="#" tabindex="-1" @click="selectPage('settings', {}, $event)">{{
          i18n.navSettings
        }}</a>
      </li>
      <li v-if="data.baseConfig.gw2Instances?.nvidiaShare?.length > 0" class="nav-item active">
        <a
        ...
        >
          <div
            ...
            :style="{
              overflow: 'hidden',
              width: `${
                (data.progressConfig.parsedLogs / (data.progressConfig.parsingLogs || 1)) * 100
              }%`,
              maxWidth: `${
                (data.progressConfig.parsedLogs / (data.progressConfig.parsingLogs || 1)) * 100
              }%`
        ...
        >
          <div
            ...
            :style="{
              overflow: 'hidden',
              width: `${
                (data.progressConfig.compressedLogs / (data.progressConfig.compressingLogs || 1)) *
                100
              }%`,
              maxWidth: `${
                (data.progressConfig.compressedLogs / (data.progressConfig.compressingLogs || 1)) *
      ...
      <li v-if="data.mumbleLinkActive?.identity" class="nav-item">
      ...
      <li class="nav-item nav-item-space active">
        <a class="nav-link" href="#" tabindex="-1" @click="selectPage('credits', {}, $event)">{{
          i18n.navCredits
        }}</a>
      </li>
    </ul>
  ...
  <div v-if="data.baseConfig.zoom !== 1" class="zoom-indicator">
